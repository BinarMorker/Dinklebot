...
class ProgressionCard {
	...
	public function display() {
		if ($this->data->progressionHash == "594203991") {
			$icon = "http://www.bungie.net".$this->defs['2161005788']['icon'];
			...
			$icon = "http://www.bungie.net".$this->defs['3233510749']['icon'];
			$percent = $this->data->level;
			$count = $this->data->level;
			$max = 100;
		} elseif ($this->data->progressionHash == "2033897755") {
			$icon = "http://www.bungie.net".$this->defs['1357277120']['icon'];
			$percent = $this->data->level;
			$count = $this->data->level;
			$max = 100;
			...
			$icon = $GLOBALS['config']->site_root."/image/50/200/100/www.bungie.net".$request->get_response()->data->destination->icon;
			...
			$icon = "http://www.bungie.net".$this->defs[(string)$this->data->progressionHash]['icon'];
