...
Data.CanvasList[b]=$("#canvas-"+b).get()[0];Data.SpasmList[b]=new Spasm.ItemPreview(Data.CanvasList[b],Url+"/proxy/www.bungie.net");$("#button-"+b).on("click",function(c){c.preventDefault();
$("#button-"+b).off("click");var d=$("#button-"+b).text();$("#button-"+b).html("<img src='"+Url+"/img/loading.gif'/>");loadCharacter(b,function(){$("#button-"+b).hide();
$("#button-"+b).text(d);$("#character-"+b).fadeOut();$("#canvas-"+b).fadeIn();});});});});console.log(Data);});function fetchGearAssets(b,d){if(Data.GearAssets==undefined){Data.GearAssets=[];
}var a=[];b.forEach(function(e){if(Data.GearAssets[e]==null){a.push(e);}});if(a.length>0){var c=a.map(function(e){return Url+"/proxy/www.bungie.net/platform/destiny/manifest/gearAsset/"+e+"/";
});getItems(c,function(g){for(var f=0;f<g.length;f++){var e=g[f];if(e.Response==null){Data.GearAssets[a[f]]={};}else{Data.GearAssets[e.Response.data.requestedId]=e.Response.data.gearAsset;
}}d(true);});}else{d(true);}}function getItems(c,d,b){if(b==undefined){b=[];}var a=new XMLHttpRequest();a.open("GET",c.shift(),true);a.onload=function(){b.push(JSON.parse(a.responseText));
...
}});if(a.length>0){var c=new XMLHttpRequest();c.open("GET",Url+"/proxy/www.bungie.net/Platform/Destiny/Manifest/InventoryItem/"+a.join(","),true);console.log(c);
c.onload=function(){var e=JSON.parse(c.response);e.forEach(function(f){Data.GearDetails[f.Response.data.requestedId]=f.Response.data.inventoryItem;});d(true);
};c.send();}else{d(true);}}function getShaderDetails(a,c){var b=new XMLHttpRequest();b.open("GET",Url+"/proxy/www.bungie.net/Platform/Destiny/Manifest/InventoryItem/"+a,true);
...
b.open("GET",Url+"/proxy/www.bungie.net/Platform/Destiny/"+a+"/Account/"+d,true);b.onload=function(){try{var e=JSON.parse(b.responseText);if(e.ErrorCode==1){Data.AccountType=e.Response.data.membershipType;
Data.Characters=e.Response.data.characters;c(true);return;}}catch(f){c(false);}};b.onerror=function(){c(false);};console.log(b);b.send();}function getArmorDetails(a){return getGearDetails(a).filter(function(b){return b.bucketTypeHash=="3448274439"||b.bucketTypeHash=="3551918588"||b.bucketTypeHash=="14239492"||b.bucketTypeHash=="20886954"||b.bucketTypeHash=="1585787867";
